...
test("메인 페이지 > 검색 탭 > 검색어에 영문 소문자, 숫자, 하이픈(-), 언더스코어(_)와 스코프(@scope/)만 사용할 수 있습니다.", async ({
  page,
}) => {
  // given: 메인 페이지에 접속한다
  await page.goto("/");
...
test("메인 페이지 > 검색 탭 > 존재하지 않는 패키지를 검색하면 404 페이지로 이동한다", async ({ page }) => {
  // given: 메인 페이지에 접속한다
  await page.goto("/");
  await page.waitForSelector("[role=tablist]");
